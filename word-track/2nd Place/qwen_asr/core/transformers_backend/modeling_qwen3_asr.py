...
def fixed_cross_entropy(
    ...
    loss = nn.functional.cross_entropy(source.transpose(1, 2), target, ignore_index=ignore_index, reduction=reduction)
    return loss
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.training and self.dropout>0:
            hidden_states = F.dropout(hidden_states, self.dropout)
        ...
        if labels is not None:
            if len(loss_kwargs)==0:
                loss = self.loss_function(
                    logits=logits, labels=labels, vocab_size=self.config.get_text_config().vocab_size, **loss_kwargs
                )
            else:
...
class Qwen3ASRForConditionalGeneration(Qwen3ASRPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Qwen3ASRConfig):
        ...
        self.thinker = Qwen3ASRThinkerForConditionalGeneration._from_config(config.thinker_config, attn_implementation=config._attn_implementation)
        self.post_init()
