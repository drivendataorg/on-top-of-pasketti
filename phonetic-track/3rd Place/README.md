...
- Install systems packages (if on linux machine)
```bash
sudo apt-get update
sudo apt-get install -y build-essential git wget curl unzip ffmpeg \
    libsndfile1 sox libsox-dev espeak-ng \
    python3.12 python3.12-venv
```
- Setup virtual environment 
```bash
python3.12 -m venv asr-env
source asr-env/bin/activate
pip install --upgrade pip wheel setuptools 
```
- Install Python requirements (includes the PyTorch cu130 build)
```bash
