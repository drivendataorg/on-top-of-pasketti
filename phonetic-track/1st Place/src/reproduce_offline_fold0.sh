#!/usr/bin/env bash
# Reproduce the 11 official phonetic model recipes on offline fold 0
# in this standalone solution repo. Output names match src/models.txt so
# ensemble.py can consume the same list.
#
# Usage:
#   bash reproduce_offline_fold0.sh
#   FORCE=1 bash reproduce_offline_fold0.sh
#   DRY_RUN=1 bash reproduce_offline_fold0.sh
#
# Env:
#   GPU=0
#   PYTHON=python
...
for rec in "${MODELS[@]}"; do
  ...
  out_dir="$RUN_ROOT/$mn/$FOLD"
  if [[ "$FORCE" != "1" && -s "$out_dir/model.pt" ]]; then
    echo "[skip] $mn fold=$FOLD has checkpoint: $out_dir/model.pt"
    continue
  fi

  cmd="PYTHONPATH=_compat:\$PYTHONPATH CUDA_VISIBLE_DEVICES=$GPU $PYTHON train.py $args --mn=$mn --fold=$FOLD --wandb=0 --root=$ROOT --ext_root=$EXT_ROOT --eval_ext_root=$EXT_ROOT $EXPORT_ARGS"
  if [[ -n "$EXTRA_ARGS" ]]; then
    cmd="$cmd $EXTRA_ARGS"
  fi
  echo "[offline fold=$FOLD] $mn"
  run_cmd "$cmd"
done
